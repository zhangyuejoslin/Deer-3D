...
from utils.basic_utils import (MetricLogger, SmoothedValue, setup_seed, ExponentialMovingAverage)
...
def train(
    ...
    for i, (media_type, batch) in enumerate(iterator):
        ...
        if do_eval and ((i+1) % eval_freq == 0 and (len(train_loader) - i >= eval_freq) or i == len(train_loader) - 1):
            ...
            val_metrics = evaluate_all(model, model_without_ddp, val_loaders, epoch, global_step, device, config, ema, use_ema_for_eval)
            if is_main_process():
...
def evaluate_all(
    ...
    # Optionally use EMA model for evaluation
    original_state_dict = None
    if use_ema_for_eval and ema is not None:
        logger.info("Using EMA model for evaluation")
        original_state_dict = ema.apply_to_model()
    ...
    if original_state_dict is not None:
        ema.restore_from_state_dict(original_state_dict)
...
def main(config):
    ...
    ema = None
    if config.get("ema", {}).get("enable", False):
        ema_decay = config.ema.get("decay", 0.9999)
        ema_update_after_step = config.ema.get("update_after_step", 0)
        logger.info(f"Initializing EMA with decay={ema_decay}, update_after_step={ema_update_after_step}")
    ...
    if config.evaluate:
        use_ema_for_eval = config.get("use_ema_for_eval", False)
        evaluate_all(model, model_without_ddp, val_loaders, start_epoch - 1, global_step, device, config, ema, use_ema_for_eval)
...
if __name__ == "__main__":
    ...
    #main(cfg)
    import sys
    if len(sys.argv) == 1:  # No arguments provided, use default config
        sys.argv.append("scripts/config.py")  # Use the default config file
